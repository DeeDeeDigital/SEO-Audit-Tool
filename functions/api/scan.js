 ...
 * Runs automated technical checks on a domain:
 *   robots.txt, sitemap, HTTPS/redirects, PageSpeed Insights
 */
export async function onRequestPost(context) {
  try {
    ...
    // Robots first — its sitemap URL feeds into the sitemap check
    ...
    const [sitemapResult, httpsResult, pagespeedResult] = await Promise.allSettled([
      ...
      checkHttps(base),
      checkPageSpeed(base, context.env?.PAGESPEED_API_KEY),
    ])
...
function settled(result) {
  return result.status === 'fulfilled' ? result.value : { error: result.reason?.message ?? 'check failed' }
}
...
async function checkRobots(base) {
  try {
    ...
    // Check if entire site is disallowed (Disallow: /)
...
async function checkHttps(base) {
  ...
  } catch { /* network error — treat as unknown */ }

  // Check final URL's www consistency + HSTS
  try {
    ...
    const origHasWww = originalHostname.startsWith('www.')
    const finalHasWww = finalHostname.startsWith('www.')
    wwwConsistent = origHasWww === finalHasWww
